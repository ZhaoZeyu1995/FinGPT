...
def test_fpb(args, model, tokenizer, prompt_fun=None):
    ...
    for i in tqdm(range(total_steps)):
        tmp_context = context[i *
                              batch_size:min((i+1) * batch_size, len(context))]
    ...
    f1_macro = f1_score(
        instructions["new_target"], instructions["new_out"], average="macro")
    f1_micro = f1_score(
        instructions["new_target"], instructions["new_out"], average="micro")
    f1_weighted = f1_score(
        instructions["new_target"], instructions["new_out"], average="weighted")

    print(
        f"Acc: {acc}. F1 macro: {f1_macro}. F1 micro: {f1_micro}. F1 weighted: {f1_weighted}. ")
...
def test_fpb_mlt(args, model, tokenizer):
    ...
    for i, template in enumerate(templates):
        ...
        dataloader = DataLoader(Dataset.from_pandas(
        ...
        for idx, inputs in enumerate(tqdm(dataloader)):
            ...
            res = model.generate(**inputs, do_sample=False, max_length=args.max_length,
    ...
    for i in range(len(templates)):
        ...
        dataset[f"out_text_{i}"] = dataset[f"out_text_{i}"].apply(
            change_target)

    dataset["new_out"] = dataset.apply(
        vote_output, axis=1, result_type="expand")
    ...
    for k in [f"out_text_{i}" for i in range(len(templates))] + ["new_out"]:
        ...
        f1_weighted = f1_score(
            dataset["target"], dataset[k], average="weighted")
