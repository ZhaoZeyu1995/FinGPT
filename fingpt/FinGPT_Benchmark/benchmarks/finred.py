...
def cvt_text_to_pred(ref, text):
    ...
    for pred_txt in text.strip('.').split(';'):
        ...
        if pred_match is not None:
            relation, word1, word2 = pred_match.group(1).strip(
            ), pred_match.group(2).strip(), pred_match.group(3).strip()
...
def calc_metric(gt_list, pred_list):
    ...
    assert true_positives + false_positives > 0, "No positive predictions"
    precision = true_positives / (true_positives + false_positives)
    recall = true_positives / (true_positives + false_negatives)

    assert precision + recall > 0, "No positive labels"
...
def test_re(args, model, tokenizer):
    ...
    dataset = load_from_disk(
        str(Path(__file__).parent.parent / 'data/fingpt-finred-re'))['test']
